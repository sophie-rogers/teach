...
/*If user is already logged in, directs to staff manage bookings page*/
if(isset($_SESSION["foundID"])) {
    header("Location:staffmanagebookings.php");
  }
...
include('DBconnect.php');
  ...
  try {
      ...
      $sql = "SELECT name, tutorId, password FROM Tutor";
      ...
      /*Pulls id, username and password from tutor table within database*/
      /*Declaration of arrays to store database data */
        $arrayPasswordStaff = array();
        $arrayNameStaff = array();
        $combinedStaff = array();
        $combinedIDStaff = array();
        $arrayIDStaff = array();

      /*Iterates through table rows from the tutor table and pushes values to arrays */
        foreach($res as $row) {
          array_push($arrayPasswordStaff, $row['password']);
          array_push($arrayNameStaff, $row['name']);
          array_push($arrayIDStaff, $row['tutorId']);
        }

      /*Declares 3 Strings storing the imploded data from the above arrays, combining them and setting session variables */
        $implodedArrayPasswordStaff = implode(' ', $arrayPasswordStaff);
        $implodedArrayNameStaff = implode(' ', $arrayNameStaff);
        $implodedArrayIDStaff = implode(' ', $arrayIDStaff);
        $combinedStaff = array_combine($arrayPasswordStaff, $arrayNameStaff);
        $combinedIDStaff = array_combine($arrayIDStaff, $arrayNameStaff);

        $combinedImplodeStaff = implode(' ', $combinedStaff);
        $combinedExplodeStaff = explode(' ', $combinedImplodeStaff);

       $_SESSION['combinedArrayStaff'] = $combinedStaff;
       $_SESSION['combinedLoginsStaff'] = $combinedIDStaff;
  ...
  <main id="main">
    ...
    <section class="sample-page">
      <div class="container" data-aos="fade-up">
          ...
          <form name="indexForm" action="staffmanagebookings.php" method="post" onsubmit="return validateForms()">
            ...
            <input type="submit" value="Login" style="
                        ...
                        <a href="staffregister.php"><input class="submitbuttons" type="button" value="Register" style="
                        font-family: var(--font-primary);
                        font-weight: 500;
                        font-size: 15px;
                        letter-spacing: 1px;
